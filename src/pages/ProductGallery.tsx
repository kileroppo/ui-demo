...
import { Monitor, Banknote, Heart, ShoppingCart, Hotel, Palette, BookOpen, Leaf, Package } from 'lucide-react'
...
function CategoryIcon({ category, className }: { category: string; className?: string }) {
  ...
  const IconComponent = LUCIDE_ICON_MAP[iconName] || Package
        ...
        <nav className="mb-6 flex flex-wrap gap-2" aria-label="产品分类导航">
          {categoryNames.map((cat) => (
            <a
              key={cat}
              href={`#category-${cat}`}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 dark:hover:text-indigo-400 hover:border-indigo-200 dark:hover:border-indigo-700 transition-colors"
            >
              <CategoryIcon category={cat} className="w-4 h-4" /> {cat} ({grouped[cat].length})
            </a>
          ))}
        </nav>
      )}
          ...
          {categoryNames.map((category) => (
            <section key={category} id={`category-${category}`} aria-label={`${category} 类别`}>
              <div className="flex items-center gap-2 mb-4">
                <CategoryIcon category={category} className="w-5 h-5 text-gray-600 dark:text-gray-400" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {grouped[category].map((product) => (
                  ...
                  >
                    ...
                    {product.colorFocus && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-500 dark:text-gray-400">配色方向</p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">{product.colorFocus}</p>
                      </div>
                    )}
